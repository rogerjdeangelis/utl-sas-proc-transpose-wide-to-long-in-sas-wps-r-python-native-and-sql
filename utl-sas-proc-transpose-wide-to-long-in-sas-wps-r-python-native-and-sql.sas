...
wide, meta = pyreadstat.read_sas7bdat("d:/sd1/have.sas7bdat");
print(wide);
sql="Select Code, Country, Y1950 As Year, ? As Value From wide";
cur.execute(sql,(yr,));
res = cur.fetchall();
print(res);
');
...
sql="Select Code, Country, Y1950 As Year, '"+ str(yr) + |"' As Value From wide"
cur.execute(sql,(yr,))
res = cur.fetchall()
print(res)
''''
...
proc sql;
  create
     table want as
    %do_over(_seq,phrase=%str(
  Select Code, Country, "Y195?" As Year, Y195? As Value From have),between=union)
;quit;
...
wide <- structure(list(Code = structure(1:2, .Label = c("AFG", "ALB"), class = "factor"),
...
run;quit;
/*     _
  __ _| | __ _ _ __    _ __ ___   __ _  ___ _ __ ___  ___
 / _` | |/ _` | `_ \  | `_ ` _ \ / _` |/ __| `__/ _ \/ __|
| (_| | | (_| | | | | | | | | | | (_| | (__| | | (_) \__ \
 \__,_|_|\__,_|_| |_| |_| |_| |_|\__,_|\___|_|  \___/|___/
